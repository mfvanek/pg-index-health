...
class KeyValueFileHealthLoggerTest {
    ...
    private final DatabaseCheck<Index> invalidIndexesCheck = (DatabaseCheck<Index>) Mockito.mock(DatabaseCheck.class);
    private final HealthLogger logger = new KeyValueFileHealthLogger(credentials, connectionFactory, haPgConnection -> databaseChecks);

    @BeforeEach
    void setUp() {
        Mockito.when(databaseChecks.getCheck(Diagnostic.INVALID_INDEXES, Index.class)).thenReturn(invalidIndexesCheck);
    }

    @Test
    void logInvalidIndexes() {
        Mockito.when(invalidIndexesCheck.check(any(PgContext.class)))
