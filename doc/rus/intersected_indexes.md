...
Из каждой пары пересекающихся индексов нужно удалить «лишний» — тот, чьи столбцы являются ведущим (левым) подмножеством столбцов другого индекса.
Такой индекс избыточен, поскольку запросы, использовавшие его, эффективно обслуживаются более широким составным индексом.

В скрипте выше индекс `idx_order_item_sku` по столбцу `(sku)` полностью перекрывается индексом `idx_order_item_sku_warehouse_id` по столбцам `(sku, warehouse_id)`, поэтому его можно удалить:

```sql
drop index concurrently if exists demo.idx_order_item_sku;
```

Для обычных таблиц используйте [drop index concurrently](https://postgrespro.ru/docs/postgresql/17/sql-dropindex),
чтобы удаление не блокировало запись в таблицу.

Для секционированных таблиц удаляйте индекс на самой секционированной (родительской) таблице — он каскадно удалится на всех секциях.
Опция `concurrently` для секционированных индексов не поддерживается:

```sql
drop index if exists demo.idx_order_item_partitioned_sku;
```

Перед удалением убедитесь, что удаляемый индекс не используется отдельно (например, как уникальный или как индекс под ограничение),
и что более широкий индекс действительно покрывает все нужные сценарии поиска.
