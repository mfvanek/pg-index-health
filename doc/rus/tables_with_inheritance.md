...
Откажитесь от наследования таблиц и перепроектируйте схему.

Если наследование использовалось для разбиения большой таблицы на части, перейдите на
[декларативное секционирование](https://postgrespro.ru/docs/postgresql/17/ddl-partitioning).

Если наследование требовалось лишь для переиспользования общего набора столбцов,
вынесите общие данные в одну таблицу, а специфичные — в связанные таблицы (через внешние ключи).

Разорвать существующую связь наследования, не пересоздавая таблицу, можно командой `no inherit`:
...
После этого дочерняя таблица станет независимой и перестанет попадать в результаты проверки.
