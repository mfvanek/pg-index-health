...
Переведите столбцы, хранящие UUID, с типа `varchar` на встроенный тип `uuid`.

Если в столбце уже лежат корректные UUID, тип можно изменить с явным приведением:
...
Для значений без дефисов (32 символа) PostgreSQL также корректно выполнит приведение к `uuid`.

Учитывайте зависимости: если на такой первичный ключ ссылаются внешние ключи, тип нужно менять согласованно
во всех связанных столбцах. Порядок действий обычно такой:

1. удалить внешние ключи, ссылающиеся на столбец;
2. изменить тип столбцов первичного и внешних ключей на `uuid`;
3. заново создать внешние ключи.

На больших таблицах `alter column ... type` переписывает таблицу под блокировкой и может занять продолжительное время —
