...
### Нет выигрыша по производительности или размеру
...
### Требует эксклюзивной блокировки при изменении размера

Изменение размера поля `varchar` требует блокировки `ACCESS EXCLUSIVE`, которая остановит все операции чтения и записи в таблицу.

## Когда использовать

Если вам нужно текстовое поле, которое выдаст ошибку, если вы вставите в него слишком длинную строку,
и вы не хотите использовать явный check-constraint, то varchar(n) — вполне подходящий тип.
...
- Используйте `CHECK CONSTRAINT`, чтобы ограничить максимальную длину.
