...
Для данных в формате jsonb можно использовать оператор DISTINCT.
Для json-полей DISTINCT не работает.
Если запрос для поля json сгенерируется с таким оператором (например, ORM), то это приведет к ошибкам.
