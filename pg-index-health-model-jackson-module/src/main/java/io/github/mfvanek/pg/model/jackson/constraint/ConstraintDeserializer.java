...
public class ConstraintDeserializer extends JsonDeserializer<Constraint> {
    ...
    public Constraint deserialize(final JsonParser p, final DeserializationContext ctxt) throws IOException {
        ...
        final String constraintName = node.get(ConstraintNameAware.CONSTRAINT_NAME_FIELD).asText();
        final String constraintType = node.get(Constraint.CONSTRAINT_TYPE_FIELD).asText();
